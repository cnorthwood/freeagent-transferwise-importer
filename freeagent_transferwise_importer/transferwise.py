...
class BorderlessAccount:
    ...
    def get_transactions(self, account_id, currency, since=None):
        ...
        response = self._make_request(
            f"v1/borderless-accounts/{account_id}/statement.json",
            params={
                "currency": currency,
                "intervalStart": since.strftime("%Y-%m-%dT%H:%M:%S.000Z"),
                "intervalEnd": datetime.datetime.utcnow().strftime(
                    "%Y-%m-%dT%H:%M:%S.000Z"
                ),
            },
        )
    ...
    def get_account_id_and_currencies(self):
        response = self._make_request(
            "v1/borderless-accounts", params={"profileId": self._get_profile_id()}
        )
        response.raise_for_status()
